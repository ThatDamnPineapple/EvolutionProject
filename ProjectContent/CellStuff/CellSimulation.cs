...
namespace EvoSim.ProjectContent.CellStuff
{
    public class CellNeatSimulation<T> : NEATSimulation where T : Cell, new()
    {
        ...
        public override GeneticAgent[] SortedAgents()
        {
            return Agents.OrderBy(n => -(n as Cell).scale).ToArray();
        }
        ...
        public override void UpdateHostStats()
        {
            ...
            neatHost.WEIGHT_SHIFT_STRENGTH = 12f;
            neatHost.WEIGHT_RANDOM_STRENGTH = 5f;

            neatHost.PROBABILITY_MUTATE_LINK = 1.0f;
            neatHost.PROBABILITY_MUTATE_NODE = 0.8f;
            neatHost.PROBABILITY_MUTATE_WEIGHT_SHIFT = 2.15f;
            neatHost.PROBABILITY_MUTATE_WEIGHT_RANDOM = 0.2f;
            neatHost.PROBABILITY_MUTATE_WEIGHT_TOGGLE_LINK = 0.08f;
